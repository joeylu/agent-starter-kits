...
1. When the user enters `Run Command` in any casing, read `agent-scripts/SCRIPTS.md` at the repository root.
2. Command matching must use the commands registered in `SCRIPTS.md`; command names are case-insensitive.
3. After a command is matched, run the local script path and execution method registered in `SCRIPTS.md`.
4. If the command does not exist, list possible matches and the current available commands.
5. Do not bypass `SCRIPTS.md` by guessing script paths directly.

# Project

1. You must read `agent-documents/PROJECT.md`; it contains the extra rules for this project.
