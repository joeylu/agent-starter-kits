...
## Write Permission
Assistant may switch from `DISCUSSION` to `WRITING` only after Owner explicitly replies with `approve changes`.

Without seeing `approve changes`, any write action is illegal.
